...
class UnconstrainedRegularisedMFRecommender(rcm.Recommender):
    """  """
    def __init__(self, **kwargs):
        ...
        self.lambda0 = kwargs.get('lambda', 0.1)
        self.convergence_error = kwargs.get('convergence_error', 0.001)
    ...
    def fit(self, data):
        ...
        while not self.converged:
            ...
            if self.verbose:
                print(np.abs(self.this_e - self.last_e))

            temp_U = self.U

            self.U = self.U * (1-self.alpha * self.lambda0) + self.alpha * E.dot(self.V)
            self.V = self.V * (1-self.alpha * self.lambda0) + self.alpha * E.T.dot(temp_U)
            ...
            if self.iterations > 0:
                ...
                if diff < self.convergence_error:
