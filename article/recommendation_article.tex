...
\begin{description}
    ...
    \item[\(R(u)\)] Function that delivers a set of all items that user \(u\) rated.
    \item [\(N(u)\)] Function that gives the set of all items that a user provided implicit feedback for.
    ...
    \item [\( \mu \in \mathbb{R}\)] The global average of the training ratings. 
    \item [\( b_u \in \mathbb{R} \)] The observed deviations of user \(u\) from the mean.
    \item [\( b_i \in \mathbb{R} \)] The observed deviations of item \(i\) from the mean.
    \item [\( r_{ui} \in \mathbb{K} \)] The rating of user \(u\) for item \(i\).
    \item [\( \hat{r}_{ui} \in \mathbb{R} \)] The estimated rating user \(u\) for item \(i\).
    ...
    \item [\( w_{ij} \in \mathbb{R}^{i \times i} \)] Global user-independent weights for the interaction between items.
    \item [\( c_{ij} \in \mathbb{R}^{i \times i}\)] Global user-independent weights for implicit preference.
    \item [\( q_i,p_u,y_j \in \mathbb{R}^f\)] Factor vectors.
    \item [\( s_{ij} \in \mathbb{R}^{i \times i} \)] The matrix holding the similarity between items \(i,j\).
    \item [\( n_{ij} \in \mathbb{R}^{i \times i} \)] The matrix counting how many times user have rated both items \(i,j\).
    \item [\( p_{ij} \in \mathbb{R}^{i \times i} \)] The matrix holding the Pearson correlation between items \(i,j\).
\end{description}
...
\begin{align}
\hat{r}_{ui} &= \mu + b_u + b_i \label{bias} \\ 
...
\paragraph{User and Item properties} Term \eqref{bias} models the basic properties of users and items without interactions as a sum of the global average \(\mu\), a user-bias \(b_u\) and an item-bias \(b_i\).
\paragraph{User and Item Interactions} Term \eqref{ui_interact} describes the interaction between user and item factors. It represents the factorization part of the model. To the user-profile-factor \(p_u\) the sum over the factors associated with the items implicitly rated by the user is added. 
\paragraph{Weighted nearest items} Term \eqref{rating_neighborhood} 
adds the sum over the nearest neighbors weighted by the global item-interaction weights.
\paragraph{Implicit preference} Term \eqref{impl} introduces the implicit preferences. For the nearest neighbors we add all user-independent weights from \(c_{ij}\).
...
\begin{align}    
    \min \sum\limits_{ (u,i,r) \in \mathcal{S} } \left( r_{ui} - \hat{r}_{ui} \right)^2 \nonumber 
\end{align}
